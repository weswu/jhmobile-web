          ...
          <dl class="app-button" @click="detail(item.orderId)">
              ...
              <span class="order_num">{{item.productTotalQuantity}}件</span>
    				...
    				<dt class="orange_txt fl">
    					<span> 订单号：{{item.orderSn}} </span> <span>
    						总金额：￥{{item.totalAmount}}&nbsp;&nbsp;&nbsp;(
    						{{item.orderStatusTxt}} ) </span> <span> 下单时间：{{item.addTime}} </span>
    					...
    					<button ng-if="order.shippingStatus" @click="shipments(item.orderId)" class="fl orange">
    						去发货</button>
    					<button ng-click="detail(item.orderId)" class="fl">查看详情</button>
    				</dt>
    			</dl>
          <mu-divider/>
        </template>
